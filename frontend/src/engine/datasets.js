/**
 * Built-in datasets — all India-specific, solver-verified.
...
export const BUILTIN_DATASETS = {
  bank_branches_tutorial: {
    ...
    description: '5 bank branches — the worked example from the MBA course. Simple 2-input, 2-output dataset ideal for manual verification and step-by-step teaching.',
    source: 'IFHE Hyderabad — MBA Course Dataset',
    sector: 'Banking', emoji: '🏦',
    suggested_model: 'CCR', suggested_orientation: 'input',
    input_cols:  ['Staff_FTEs', 'Operating_Cost_Lakhs'],
    output_cols: ['Loans_Processed', 'Deposits_Cr'],
    teaching_notes: 'Branches A, C, E are efficient (θ=1.0). Branch B scores θ=0.90 (10% inefficient). Branch D scores θ=0.686 (31.4% inefficient — strongest teaching example). Use Branch D to demonstrate the two-stage improvement calculation.',
    data: [
  ...
  indian_psu_banks: {
    ...
    description: '20 Indian PSU banks evaluated on the intermediation approach. Inputs: deposits, labour costs, fixed assets. Outputs: advances, investments.',
    source: 'RBI Basic Statistical Returns (BSR) — Illustrative',
    sector: 'Banking', emoji: '🏛️',
    suggested_model: 'BCC', suggested_orientation: 'input',
    input_cols:  ['Deposits_CrRs', 'Labour_Cost_CrRs', 'Fixed_Assets_CrRs'],
    output_cols: ['Advances_CrRs', 'Investments_CrRs'],
    teaching_notes: 'Use BCC model due to significant size variation (SBI vs small PSBs). Compare OTE (CCR) vs PTE (BCC) to reveal scale inefficiency.',
    data: [
  ...
  district_hospitals: {
    ...
    source: 'NITI Aayog Health Data — Illustrative',
    sector: 'Healthcare', emoji: '🏥',
    suggested_model: 'BCC', suggested_orientation: 'input',
    input_cols:  ['Doctors_FTE','Nurses_FTE','Beds','Expenditure_LakhsRs'],
    output_cols: ['OPD_Annual','IPD_Annual','Surgeries_Annual'],
    teaching_notes: 'BCC essential — huge size variation (800 beds vs 100 beds). Good for discussing case-mix adjustment and environmental heterogeneity.',
    data: [
  ...
  domestic_airlines: {
    ...
    description: '8 Indian domestic carriers. Inputs: fleet size, employees, fuel cost. Outputs: Revenue Passenger Km, on-time performance.',
    source: 'DGCA Annual Statistics — Illustrative',
    sector: 'Aviation', emoji: '✈️',
    suggested_model: 'CCR', suggested_orientation: 'input',
    input_cols:  ['Fleet_Size','Employees','Fuel_Cost_CrRs'],
    output_cols: ['RPK_Millions','OTP_Percent'],
    teaching_notes: 'Good for discussing LCC vs full-service efficiency and the strategic risk of frontier status (Discussion Q5).',
    data: [
  ...
  hotel_properties: {
    ...
    description: '12 hotel properties. Inputs: rooms, staff, energy cost. Outputs: RevPAR, occupancy rate, F&B revenue.',
    source: 'FHRAI India Hotel Survey — Illustrative',
    sector: 'Hospitality', emoji: '🏨',
    suggested_model: 'BCC', suggested_orientation: 'output',
    input_cols:  ['Rooms','Staff_FTE','Energy_Cost_LakhsRs'],
    output_cols: ['RevPAR_Rs','Occupancy_Pct','FnB_Revenue_LakhsRs'],
    teaching_notes: 'Use output-orientation — rooms are fixed in the short run. Asset-light brands (OYO, Ginger) show high efficiency despite small scale.',
    data: [
  ...
  engineering_colleges: {
    ...
    description: '18 autonomous engineering colleges. Inputs: faculty, lab area, library budget. Outputs: placement %, research publications, pass rate.',
    source: 'NIRF Rankings Data — Illustrative',
    sector: 'Education', emoji: '🎓',
    suggested_model: 'BCC', suggested_orientation: 'input',
    input_cols:  ['Faculty_FTE','Lab_Area_SqFt','Library_Budget_LakhsRs'],
    output_cols: ['Placement_Pct','Publications_Annual','Pass_Rate_Pct'],
    teaching_notes: 'Education sector: NIRF weights quality differently. Good for discussing appropriate output selection and limitations of placement % as the sole output.',
    data: [
...
export function listDatasets() {
  return Object.values(BUILTIN_DATASETS).map(ds => ({
    id: ds.id, name: ds.name, description: ds.description,
    sector: ds.sector, emoji: ds.emoji,
    n_dmus: ds.data.length,
    n_inputs: ds.input_cols.length,
    n_outputs: ds.output_cols.length,
    suggested_model: ds.suggested_model,
  }));
}

export function getDataset(id) { return BUILTIN_DATASETS[id] || null; }
